...
For simplicity, we will only deal with UTC datetimes in this example. After we
finished the custom timetable, we should be able to pass it to the DAG:

.. code-block:: python

    with DAG(timetable=AfterWorkdayTimetable(), ...) as dag:
        ...
...
.. code-block:: python
    ...
    class SometimeAfterWorkdayTimetable(Timetable):
        ...
        def next_dagrun_info(self, last_automated_dagrun, restriction):
            ...
            return DagRunInfo(
                data_interval=DataInterval(start=start, end=end),
                run_after=DateTime.combine(end.date(), self.schedule_at).replace(
                    tzinfo=end.tzinfo
                ),
            )
...
.. code-block:: python
    ...
    def serialize(self) -> Dict[str, Any]:
        return {"schedule_at": self.schedule_at.isoformat()}
